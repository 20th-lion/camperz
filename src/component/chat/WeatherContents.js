...
export default function WeatherContents() {
	...
	let sunrisedate =
		myDate2.getHours().toString().padStart(2, '0') + '시' + myDate2.getMinutes().toString().padStart(2, '0') + '분';

	const sunsettime = weather.sunset;
	let myDate3 = new Date(sunsettime * 1000);
	let sunsetdate =
		myDate3.getHours().toString().padStart(2, '0') + '시' + myDate3.getMinutes().toString().padStart(2, '0') + '분';

	const selectIcon = () => {
		if (weather.id >= 801) {
			return <S_WeathrImg src={Cloud} alt="구름"></S_WeathrImg>;
		}
